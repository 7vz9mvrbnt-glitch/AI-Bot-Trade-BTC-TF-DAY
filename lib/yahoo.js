...
async function fetchCandles(symbol, limit = 50) {
  ...
  const candles = await fetchChart(symbol, "3mo", "1d");
  return candles.slice(-limit);
}

/**
 * @param {string} symbol
 * @param {string} range     Yahoo range, e.g. "3mo", "5y"
 * @param {string} interval  Yahoo interval, e.g. "1d", "1wk"
 */
...
module.exports = { fetchCandles, fetchChart };
