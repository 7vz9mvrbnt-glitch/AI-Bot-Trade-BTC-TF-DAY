...
function buildAIComment(setup) {
  ...
  if (mode !== "indicator" && fibLevels) {
    ...
    const nearRes = fibArr.filter(x => x.val > price).sort((a, b) => a.val - b.val)[0];
    const nearSup = fibArr.filter(x => x.val < price).sort((a, b) => b.val - a.val)[0];
    const resPrice = nearRes ? nearRes.val : swingHigh;
    const resLabel = nearRes ? `Fib ${nearRes.label}` : "Swing High";
    const supPrice = nearSup ? nearSup.val : swingLow;
    const supLabel = nearSup ? `Fib ${nearSup.label}` : "Swing Low";
    lines.push(`🎯 แนวต้านใกล้: $${fmt(resPrice)} (${resLabel})`);
    lines.push(`📍 แนวรับใกล้:  $${fmt(supPrice)} (${supLabel})`);
  }
