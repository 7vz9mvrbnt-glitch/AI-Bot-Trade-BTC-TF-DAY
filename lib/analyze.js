...
function analyze(candles, symbol = "BTCUSDT", source = "binance", mode = null) {
  ...
  // Fibonacci — คำนวณเฉพาะ accumulate mode
  let fibLevels = null, swingHigh = null, swingLow = null, nearFib = null;
  if (mode === "accumulate") {
    const sp = findSwingPoints(candles);
    swingHigh = sp.swingHigh;
    swingLow  = sp.swingLow;
    fibLevels = calcFibLevels(swingHigh, swingLow);
    nearFib   = nearestFibZone(price, fibLevels);
  }
