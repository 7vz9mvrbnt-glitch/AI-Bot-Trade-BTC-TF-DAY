...
function calcRSI(closes, period = 14) {
  ...
  if (value >= 75) {
    label = "Overbought — แพงเกินไป"; emoji = "🔴"; color = "#dc2626"; zone = "overbought";
  } else if (value >= 60) {
    label = "High — เริ่มแพง ระวัง";  emoji = "🟠"; color = "#f97316"; zone = "high";
  } else if (value >= 40) {
    label = "Neutral — ปกติ";          emoji = "🟡"; color = "#eab308"; zone = "neutral";
  } else if (value >= 30) {
    label = "Low — เริ่มถูก น่าสนใจ"; emoji = "🟢"; color = "#16a34a"; zone = "low";
  } else {
    label = "Oversold — ถูกมาก น่าสะสม"; emoji = "🟢"; color = "#166534"; zone = "oversold";
  }
...
function calcMACD(closes) {
  ...
  let label, emoji;
  if (cross === "bullish") {
    label = "Bullish Cross — โมเมนตัมกลับบวก"; emoji = "🟢";
  } else if (cross === "bearish") {
    label = "Bearish Cross — โมเมนตัมอ่อนลง";  emoji = "🔴";
  } else if (histogram > 0) {
    label = "Bullish — แรงซื้อยังมีอยู่";       emoji = "🟡";
  } else {
    label = "Bearish — แรงซื้อลดลง";            emoji = "🟠";
  }
...
function calcFibLevels(swingHigh, swingLow) {
  ...
  const retr = (pct) => parseFloat((swingHigh - range * pct).toFixed(2));
  const ext  = (mul) => parseFloat((swingLow  + range * mul).toFixed(2));
  return {
    // Retracement — แนวรับ (ปรับตัวลง)
    f236: retr(0.236), f382: retr(0.382), f500: retr(0.500),
    f618: retr(0.618), f650: retr(0.650), f786: retr(0.786),
    // Extension — แนวต้าน/เป้ากำไร (วิ่งขึ้นเกิน swingHigh)
    // Formula: swingLow + range × multiplier
    fE100:  ext(1.000),  // 100%  = swingHigh (เป้าขั้นต่ำ)
    fE1272: ext(1.272),  // 127.2% = √1.618 (เป้ากลาง)
    fE1618: ext(1.618),  // 161.8% Golden Ratio ★ เป้าหลัก
    fE2618: ext(2.618),  // 261.8% (Bull Run)
  };
}
...
function buildAIComment(setup) {
  ...
  // สถานะตลาด
  if (marketClosed) {
    lines.push(`🔒 ตลาดปิดอยู่ — ราคาล่าสุด ณ ${lastTradeDate}`);
    if (nextOpenICT) lines.push(`⏰ ตลาดจะเปิดอีกครั้ง: ${nextOpenICT}`);
    lines.push(`📋 ทิศทางล่าสุดก่อนปิด:`);
  }
  ...
  if (mode !== "indicator" && fibLevels) {
    const retrArr = [
      { label: "23.6%", val: fibLevels.f236 },
      { label: "38.2%", val: fibLevels.f382 },
    ...
    const extArr = [
      { label: "Ext 100%",   val: fibLevels.fE100  },
      { label: "Ext 127.2%", val: fibLevels.fE1272 },
      { label: "Ext 161.8%", val: fibLevels.fE1618 },
      { label: "Ext 261.8%", val: fibLevels.fE2618 },
    ];
    // แนวต้าน: ถ้าราคาเกิน swingHigh แล้ว ใช้ Extension; ถ้ายังไม่ถึง ใช้ Retracement ที่อยู่เหนือราคา
    const nearRes = price > swingHigh
      ? extArr.filter(x => x.val > price).sort((a, b) => a.val - b.val)[0]
      : retrArr.filter(x => x.val > price).sort((a, b) => a.val - b.val)[0] || { label: "Swing High", val: swingHigh };
    // แนวรับ: Retracement ที่ใกล้ราคาที่สุด (ต่ำกว่าราคา)
    const nearSup = retrArr.filter(x => x.val < price).sort((a, b) => b.val - a.val)[0]
      || { label: "Swing Low", val: swingLow };
    lines.push(`🎯 แนวต้านใกล้: $${fmt(nearRes.val)} (${nearRes.label})`);
    lines.push(`📍 แนวรับใกล้:  $${fmt(nearSup.val)} (Fib ${nearSup.label})`);
  }
  ...
  // ความร้อนแรงตลาดและจังหวะน่าสนใจ
  if (rsi && setup.signalScore) {
    const heatMap = { overbought: "🔥🔥 ร้อนจัด — เสี่ยงปรับฐาน", high: "🔥 ร้อน — ระวังแรงขาย", neutral: "😊 สมดุล — ตลาดพักตัว", low: "❄️ เย็น — ใกล้โซน DCA", oversold: "🧊 เย็นจัด — น่าสะสมมาก" };
    const heat = heatMap[rsi.zone] || "😊 ปกติ";
    lines.push(`\n💹 ความร้อนแรงตลาด: ${heat} (RSI ${rsi.value})`);
    if (setup.signalScore.stars >= 4) {
      lines.push(`⭐ จังหวะน่าสนใจ — Signal ${setup.signalScore.emoji} ${setup.signalScore.label}`);
    } else if (setup.signalScore.stars <= 2) {
      lines.push(`💤 สัญญาณยังอ่อน — ไม่เร่งตัดสินใจ รอให้ชัดขึ้นก่อน`);
    }
  }
