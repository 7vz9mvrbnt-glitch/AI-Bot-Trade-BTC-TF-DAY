...
function buildSetupFlex(setup) {
  ...
  // AI comment — ตัดให้เหลือ 2 บรรทัด
  const aiLines = (setup.aiComment || "").split("\n").filter(Boolean).slice(0, 2);
        ...
        contents: [
          ...
          ...(aiLines.length > 0 ? [
            { type: "separator", margin: "sm" },
            {
              ...
              paddingAll: "10px",
              contents: [
                { type: "text", text: "💬 AI วิเคราะห์", size: "xxs", color: "#16a34a", weight: "bold" },
                ...aiLines.map(line => ({
