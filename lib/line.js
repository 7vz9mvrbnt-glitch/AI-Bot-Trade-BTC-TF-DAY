...
/**
 * Build a Flex Message "bubble" showing BTC daily setup
 */
function buildSetupFlex(setup) {
  const trendColor = setup.trend === "UP" ? "#00C851" : "#FF4444";
  const recColor =
    setup.recommendation === "BUY" ? "#00C851"
    : setup.recommendation === "SHORT" ? "#FF4444"
    : "#FFBB33";

  return {
    type: "flex",
    altText: `BTC Setup ${setup.recommendation} | ${setup.price.toLocaleString()} USD`,
    contents: {
      type: "bubble",
      header: {
        type: "box",
        ...
        contents: [
          { type: "text", text: `BTC/USDT · Daily`, color: "#FFFFFF", size: "sm" },
        ...
        contents: [
          {
            ...
            action: {
              ...
              // จะเปลี่ยน URL นี้หลัง deploy Vercel
