...
function buildSetupFlex(setup) {
  ...
  const hasTrade = isTradeMode && setup.sl && setup.tp && setup.entryLow && setup.entryHigh;
  ...
  const aiLines = isIndicator
    ? allAiLines.filter(l => l.includes("ความหมาย") || l.includes("⚠️") || l.includes("✅") || l.includes("💡")).slice(0, 5)
